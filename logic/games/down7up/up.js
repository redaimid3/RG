...
module.exports = async function (db, vk, context, limits) {
    ...
    if (db.gamesData[context.peerId].convData.isActive == true && db.gamesData[context.peerId].convData.gamemode == 'down7up' && context.messagePayload && context.messagePayload.command == 'down7up_bet_up') {
        ...
        for (i in db.gamesData) {
            if (db.gamesData[i].convGame.bets[context.senderId] && db.gamesData[i].convGame.bets[context.senderId].up && db.gamesData[i].convData.isActive == true && db.gamesData[i].convData.gamemode == 'down7up') {
                currentStavka += Math.floor(db.gamesData[i].convGame.bets[context.senderId].up)
                console.log(`ok`, db.gamesData[i].convGame.bets[context.senderId].up)
            }
        }
        ...
        if (!db.gamesData[context.peerId].convGame.bets[context.senderId].up) {
            db.gamesData[context.peerId].convGame.bets[context.senderId].up = 0
        }

        db.gamesData[context.peerId].convGame.bets[context.senderId].up += Math.floor(_coin)
        ...
        if (Number(db.playersData[context.senderId].balance) >= Number(_coin)) {
            ...
            context.send(`${db.playersData[context.senderId].userData.globalSettings.allowCallNickname == true ? `[id${context.senderId}|${db.playersData[context.senderId].name}]` : `${db.playersData[context.senderId].name}`}, успешная ставка ${util.number_format(_coin)} коинов на выпадение числа больше 7`)
